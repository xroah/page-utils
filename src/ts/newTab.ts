...
function updateSentence(sentence: any) {
...
function fetchWeather() {
    function handleRes(res: any) {
        ...
        let pName:HTMLElement = el.querySelector(".pollution-name");
        ...
        for (let i of imgs) {
            if (new RegExp(`^${i}`).test(today.wea)) {
                img.src = `${chrome.runtime.getURL("")}icons/weather/${i}.png`;
                break;
            }
        }
    ...
    request("http://pv.sohu.com/cityjson", "text").then((res: string) => {
        let reg = /{.*}/;
